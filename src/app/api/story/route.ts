...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    choices: {
      ...
      items: { type: 'string' },
      description:
        'プレイヤーが行動を思いつくための「行動例」を3個程度。プレイヤーは自由入力するので、あくまでヒントとなる短い例にする。結末のときは空配列',
    },
...
- プレイヤーは選択肢ではなく「自由なテキスト」で行動を入力する。場面の最後は、プレイヤーが次に何をするか想像しやすいように、選択の余地が残る開かれた状況で終える（「どうする？」と問いかける必要はない）。
- choices には、行動に迷わないための「行動例」を3個ほど添える。これは menu ではなくヒントであり、プレイヤーはこれ以外の行動も自由に入力できる。
- プレイヤーの入力が世界観や状況にそぐわない場合でも、頭ごなしに否定せず、物語として自然に受け止める。不可能な行動なら、その理由を情景として描写したうえで物語を前に進める。
...
function buildUserMessage(
  ...
  const instruction =
    history.length === 0
      ? `冒険の幕開けとなる最初の場面（floor 1）を描写してください。世界に主人公を引き込む導入にし、プレイヤーが最初の一手を考えられる状況で終えること。`
      : `プレイヤーは直前の場面で「${history[history.length - 1].action}」という行動を取りました。その行動の結果として次の場面（floor ${floor}）を描写してください。`;
